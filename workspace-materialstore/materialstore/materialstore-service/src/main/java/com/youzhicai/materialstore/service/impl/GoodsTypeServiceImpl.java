...
public class GoodsTypeServiceImpl implements GoodsTypeService {
     ...
     * @return
     * @see com.youzhicai.materialstore.service.GoodsTypeService#findGoodsTypeListForSearch(com.youzhicai.materialstore.
...
//            goodsTypeVO.setSubGoodsTypes(POUtil.convert(findSubtypeList, GoodsTypeVO.class, TypeTokens.GoodsTypeVO));
     ...
     * @return
     * @see
     * com.youzhicai.materialstore.service.GoodsTypeService#deleteType(com.youzhicai.materialstore.dto.GoodsTypeDTO)
     */
    @Transactional
    @Override
    public boolean deleteType(GoodsTypeDTO goodsTypeDTO) {
        ...
        String companyId = goodsTypeDTO.getCompanyId();
        Long thirdId = goodsTypeDTO.getThirdId();
        if (thirdId == null || thirdId == 0 || StringUtils.isEmpty(companyId)) {
            return result;
        }

        // 查询三级分类是否存在
        Map<String, Object> queryGoodsTypeCodesMap = new HashMap<String, Object>();
        queryGoodsTypeCodesMap.put("thirdRankId", thirdId);
        queryGoodsTypeCodesMap.put("companyId", companyId);
        Map<String, Object> allRanks = goodsTypeDAO.findAllRankCodes(queryGoodsTypeCodesMap);
        if (allRanks == null || allRanks.get("firstRankId") == null || allRanks.get("secondRankId") == null || allRanks.get("thirdRankId") == null) {
            return result;
        }

        String updateId = goodsTypeDTO.getUpdateId();
        String updator = goodsTypeDTO.getUpdator();

        // 删除三级分类(若未被使用)
        int modifyThirdRankUnAble = goodsTypeDAO
                .modifyThirdRankUnAble(deleteParamsMap((Long) allRanks.get("thirdRankId"), companyId, updateId, updator));

        if (modifyThirdRankUnAble > 0) {
            result = true;// 三级分类删除成功

            // 删除二级分类(若无可用三级分类)
            int modifySecondRankUnAble = goodsTypeDAO
                    .modifySecondRankUnAble(deleteParamsMap((Long) allRanks.get("secondRankId"), companyId, updateId, updator));

            if (modifySecondRankUnAble > 0) {
                // 删除一级分类(若无可用二级分类)
                goodsTypeDAO.modifyFirstRankUnAble(deleteParamsMap((Long) allRanks.get("firstRankId"), companyId, updateId, updator));
            }
        }
        return result;
    }

    private Map<String, Object> deleteParamsMap(Long rankId, String companyId, String updateId, String updator) {
